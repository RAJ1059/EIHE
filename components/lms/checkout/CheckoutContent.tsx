...
type CheckoutContentProps = {
   ...
   * page-background section — the portal already provides one). */
  variant: "public" | "portal";
  browseCoursesPath: string;
  /** Where "Returning customer? Log in" sends a guest back to after
   * logging in. Irrelevant inside the portal — a student is always
   * already logged in there, so that banner never renders. */
  loginRedirectPath: string;
};

export function CheckoutContent({ variant, browseCoursesPath, loginRedirectPath }: CheckoutContentProps) {
  ...
  if (items.length === 0) {
    const empty = (
      <div className={isPortal ? "text-center" : "mx-auto max-w-2xl px-6 py-16 text-center"}>
    ...
    return isPortal ? empty : <section className="bg-cream">{empty}</section>;
  }
...
function CheckoutForm({
  ...
  // back into the course after paying. Never triggers inside the portal,
  // since a student there is always already logged in.
    ...
    <div className={isPortal ? "" : "mx-auto max-w-4xl px-6 py-16"}>
      ...
      {needsAccount && (
        <div className="mt-6 rounded-lg border-l-4 border-teal bg-teal/10 px-5 py-3 text-sm text-ink">
          Returning customer?{" "}
          <Link
            href={`/login?next=${loginRedirectPath}`}
