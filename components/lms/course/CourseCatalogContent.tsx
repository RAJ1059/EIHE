...
export function CourseCatalogContent({
  ...
  /** "portal" renders inline inside the Student portal shell. */
  ...
  const body = (
    <div className={isPortal ? "" : "mx-auto max-w-7xl px-6 py-16 lg:px-10 lg:py-24"}>
      <h1 className="text-3xl font-extrabold tracking-tight text-sage sm:text-4xl">
        {isPortal ? "All Courses" : "Courses"}
      </h1>
      ...
      >
        <input
          ...
          className={`w-full rounded-lg border border-ink/10 px-4 py-2.5 text-sm text-ink placeholder:text-ink/40 focus:border-teal focus:outline-none ${isPortal ? "bg-white" : "bg-cream"}`}
        />
        <button
          type="submit"
      ...
      {courses === null && !error && (
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className={`h-64 animate-pulse rounded-2xl ${isPortal ? "bg-white" : "bg-cream"}`} />
          ))}
        </div>
      )}
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {courses.map((course) => (
            <Link
              ...
              className={`flex flex-col overflow-hidden rounded-2xl border border-ink/5 shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl ${isPortal ? "bg-white" : "bg-cream"}`}
            >
  ...
  return <section className="bg-white">{body}</section>;
}
